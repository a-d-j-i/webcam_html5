...
Html5video.toBlackWhitePA = function(inputPA, outputPA, w, h) {
...
Html5video.toSepiaPA = function(inputPA, outputPA, w, h) {
	// var A = $.inim.canvasIoFactory(canvas);
	// var ctx = A[0];
	var inputData = inputPA.data;
	var outputData = outputPA.data;
	...
	for (var y = 0; y < h; y += 1) {
		for (var x = 0; x < w; x += 1) {
			...
			outputData[pixel + 3] = inputData[pixel + 3];
			/*
			 * var pixel = (y*w + x)*4; var factor = ((inputData[pixel] *.3 +
			 * inputData[pixel+1]*.59 + inputData[pixel+2]*.11) );
			 * outputData[pixel] = factor * 1; outputData[pixel+1] = factor * 1;
			 * outputData[pixel+2] = factor * 1; outputData[pixel+ 3] =
			 * inputData[pixel+ 3];
			 */
		}
	}
...
Html5video.toInvertPA = function(inputPA, outputPA, w, h) {
    var inputData = inputPA.data;
    var outputData = outputPA.data;
    /*var cr = r/255;
    var cg = g/255;
...
Html5video.toRgbPA = function(inputPA, outputPA, w, h) {
    //var A = $.inim.canvasIoFactory(canvas);
    //var ctx = A[0];
    var inputData = inputPA.data;
    var outputData = outputPA.data;
...
Html5video.toBWOutlinePA = function(inputPA, outputPA, w, h) {
    var inputData = inputPA.data;
    var outputData = outputPA.data;
    outputData =  Html5video.doEdgeDetectionAlgorithm(inputData,255, 1, outputData,w,h, 8);
    var pixel = 0;
    for (var y = 0; y < h; ++y) {
        for (var x = 0; x < w; ++x) {
...
Html5video.toMatrixOutlinePA = function(inputPA, outputPA, w, h) {
    var inputData = inputPA.data;
    var outputData = outputPA.data;
    outputData =  Html5video.doEdgeDetectionAlgorithm(inputData,255, 1, outputData,w,h, matrixrange);
    var pixel = 0;
    for (var y = 0; y < h; ++y) {
        for (var x = 0; x < w; ++x) {
            if( outputData[pixel] < 240 ) {
...
Html5video.doEdgeDetectionAlgorithm = function (inputData, baseColor, grey, outputData, w, h, range) {
